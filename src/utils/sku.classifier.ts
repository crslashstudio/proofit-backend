 ...
 */
export function classifySku(input: ClassifyInput): SkuClassification | null {
  ...
  if (netProfit < 0) return "cash_burner";
  if (isHighMargin && isHighVolume) return "star";
  if (isHighMargin && !isHighVolume) return "hidden_gem";
  if (isHighVolume && isLowMargin) return "illusion_bestseller";
...
export function computeRiskScore(
  ...
  if (classification === "cash_burner") return Math.min(100, 70 + Math.abs(netProfit) * 0.01);
  if (classification === "illusion_bestseller") return Math.min(100, 50 + Math.max(0, 5 - marginPercent));
  if (classification === "hidden_gem") return Math.max(0, 30 - marginPercent * 0.5);
  if (classification === "star") return Math.max(0, 20 - marginPercent * 0.3);
  return 50;
}
